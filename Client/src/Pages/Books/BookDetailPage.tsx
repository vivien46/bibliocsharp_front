...
const BookDetailPage: React.FC = () => {
    ...
    const getPreviousBook = async () => {
        try {
          const response = await fetch(`${API_URL}/book/previous/${id}`);
          if (response.ok) {
      ...
      const getNextBook = async () => {
        try {
          const response = await fetch(`${API_URL}/book/next/${id}`);
          if (response.ok) {
            const data = await response.json();
            navigate(`/book/${data.id}`);
          }
    ...
    if (!bookData) {
        return <div className="alert alert-danger">Pas de données trouvées</div>;
    }
